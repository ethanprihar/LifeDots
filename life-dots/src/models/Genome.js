...
{
    constructor(ally_min, ally_max, max_size, baby_frac, eat_ratio, speed, weights)
    {
    ...
    mutate(max_mut_pct)
    {
        let new_max_size = this.pct_mut(this.max_size, max_mut_pct);
        if (new_max_size <= 0) {return null;}
        let new_baby_frac = this.pct_mut(this.baby_frac, max_mut_pct);
        if ((new_baby_frac <= 0) || (new_baby_frac >= 1)) {return null;}
        let new_speed = this.pct_mut(this.speed, max_mut_pct);
        if (new_speed <= 0) {return null;}
        let new_ally_min = this.pct_mut(this.ally_min, max_mut_pct);
        let new_ally_max = this.pct_mut(this.ally_max, max_mut_pct);
        if (new_ally_min >= new_ally_max) {return null;}
        let new_eat_ratio = Math.min(Math.max(this.fix_mut(this.eat_ratio, max_mut_pct), 0), 1);
        ...
        ops.random(rand);
        ops.subseq(rand, 0.5);
        ops.mulseq(rand, 2 * max_mut_pct);
        ops.addeq(new_weights, rand);
        return new Genome(new_ally_min, new_ally_max, new_max_size, new_baby_frac, new_eat_ratio, new_speed, new_weights);
    }

    pct_mut(gene, max)
    {
        let rand = Math.random() * 2 - 1;
        return gene + rand * gene * max;
    }

    fix_mut(gene, max)
    {
        let rand = Math.random() * 2 - 1;
        return gene + rand * max;
    }
}
